...
def test_start_up_with_fresh_config(qtbot, mocker, fresh_config):
    ...
    mocked_getConfig = mocker.patch(
        "addon.addonWindow.mw.addonManager.getConfig", return_value=fresh_config
    )
    w = Windows()
    qtbot.addWidget(w)
    qtbot.wait(200)
    ...
    assert (
        w.usernameLineEdit.text()
        == w.passwordLineEdit.text()
        == w.cookieLineEdit.text()
        == ""
    )
...
def test_version_check(qtbot, mocker, monkeypatch):
    mocked_VersionCheckWorker_run = mocker.patch(
        "addon.addonWindow.VersionCheckWorker.run"
    )
...
def test_get_deck_list(qtbot, fresh_config, mocker):
    ...
    qtbot.addWidget(w)
    qtbot.wait(200)
    assert [w.deckComboBox.itemText(row) for row in range(w.deckComboBox.count())] == [
        "a",
        "b",
        "c",
    ]
...
@pytest.mark.skip
@pytest.mark.parametrize("words", [["a", "b", "c", "d"], []])
def test_newWordWidget(window, words):
    window.insertWordToListWidget(words)
    assert [
        window.newWordListWidget.item(row).text()
        for row in range(window.newWordListWidget.count())
    ] == words
    assert all(
        window.newWordListWidget.item(row).data(Qt.ItemDataRole.UserRole) is None
        for row in range(window.newWordListWidget.count())
    )
...
def test_fetch_word_and_compare(
    monkeypatch, mocker, window, qtbot, local_words, remote_words, test_index
):
    mocker.patch(
        "addon.dictionary.eudict.Eudict.getWordsByPage", return_value=remote_words
    )
    ...
    qtbot.wait(1000)
    item_in_list_widget = [
        window.newWordListWidget.item(row)
        for row in range(window.newWordListWidget.count())
    ]
    item_in_del_widget = [
        window.needDeleteWordListWidget.item(row)
        for row in range(window.needDeleteWordListWidget.count())
    ]
    words_in_list_widget = [i.text() for i in item_in_list_widget]
    words_in_del_widget = [i.text() for i in item_in_del_widget]

    assert all(
        [item.data(Qt.ItemDataRole.UserRole) is None for item in item_in_list_widget]
    )
    if test_index == 0:
