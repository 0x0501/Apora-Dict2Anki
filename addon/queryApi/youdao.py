...
class Parser:
    ...
    def definition(self) -> list:
        ...
        try:
            ec = [
                d["tr"][0]["l"]["i"][0] for d in self._result["ec"]["word"][0]["trs"]
            ][:3]
        except KeyError:
            ec = []
        # Web trans
        try:
            web_trans = [
                w["value"]
                for w in self._result["web_trans"]["web-translation"][0]["trans"]
            ][:3]
        except KeyError:
            web_trans = []
        ...
        try:
            return [
                (
                    ...
                    SENTENCE_SPEECH_URL_PREFIX + s["sentence-speech"],
                )
                for s in self._result["blng_sents_part"]["sentence-pair"]
            ][:3]
        except KeyError:
            return []
